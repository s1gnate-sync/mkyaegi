...
func main() {
	...
	replAfterScript := flags.Bool("repl", os.Getenv("YAEGI_REPL") != "", "enter repl after script, environment YAEGI_REPL can be also used")
	tags := flags.String("tags", "", "build tags")
	packages := 0
	flags.BoolFunc("packages", "list included packages, supply twice too show full name", func(_ string) error {
		packages++
		return nil
	})
	exports := flags.String("exports", "", "list exported names")
	...
	if (script == "-" && "" == *eval) || *replAfterScript {
		if !*keepInitMain {
